...
- **Absent** (official `postgres:18`, the Bitnami subchart): the migration raises a WARNING and installs the plain-SQL fallback **`dle_click_events_maintain()`**, which pre-creates 7 days of partitions and drops those past retention. It must run **daily**: `pg_cron` (`SELECT cron.schedule('dle-partitions', '5 0 * * *', 'SELECT dle_click_events_maintain()')`), a host cron running `psql -c 'SELECT dle_click_events_maintain();'`, or a Kubernetes CronJob with the same statement against the primary.
