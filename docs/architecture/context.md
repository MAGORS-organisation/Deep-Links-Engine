...
| Customer's app + SDK | `/v1/resolve`, `/v1/events` on the control host, with an SDK key | the deferred context, `match_type`, `confidence` |
| Marketer / operator | the console at `/admin/` (OIDC) or `/api/v1` (tenant API key) | CRUD, reports, the honest attribution split |
...
Nothing outside the box is called synchronously by the edge. The only outbound traffic from the engine is webhook delivery and the domain verifier's checks of the operator's own domains — both from the control process.
...
| Valkey 8 | `valkey:8` | 6379 (internal) | — | cache, iOS deferred context (TTL 1 h) |
| ClickHouse | optional | internal | fed from PostgreSQL | analytics mirror |

Profile A (Compose) runs 2 edge replicas and 1 control on 2 vCPU / 4 GB / 20 GB for ~1 000 req/s; Profile B (Helm) runs the edge under an HPA (3–20 pods), 2 control pods, PostgreSQL with read replicas the edge reads from, a 3-shard Valkey cluster, and an OTel collector ([§B.8](../zadanie.md#b8-topológia-nasadenia)). The concrete files are in [../../deploy/README.md](../../deploy/README.md).
...
| **TB4** | the control plane and the key ring | signing keys are generated and rotated by C-12; private keys never leave the internal zone; the public halves are published at `/.well-known/jwks.json`. The suite found that rotation could not retire the bootstrap key — fixed ([ADR-0013](../adr/0013-crypto-agility-from-day-one.md)) |

The full STRIDE register is [§E.2.2](../zadanie.md#e22-register-hrozieb); the compliance view (consent modes, IP hashing with a daily-rotated salt, no third party on the resolve path) is in [../compliance](../compliance).

## What is verified

Both hosts start with their route tables; the 404/410/302 behaviour on TB1 and the SDK-key scope on TB2 are exercised by the 850-test security suite through `WebApplicationFactory`. The NetworkPolicy and the Caddy routing are files under `deploy/` that have been reviewed, not deployed, on the build machine.
