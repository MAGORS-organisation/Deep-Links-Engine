...
The documents describe the design as built and do not claim more than has been checked. As of this revision, in CI: the .NET solution (14 projects) builds with `-warnaserror` at 0/0; 1 430 unit, 75 contract, 850 security and 219 integration tests pass (the last against PostgreSQL 18 and Valkey 8 in Testcontainers, including the migration, its rollback and the chaos scenarios); both hosts start; the web SDK passes 145 tests at 8.85 kB gzip; the admin console builds; the Android and iOS SDKs compile and pass their unit tests; both container images build, pass Trivy and serve an OWASP ZAP baseline. Written but **not** verified: the k6 load profile (never run) and the 8-device manual matrix (pending). Coverage is ≥ 90 % in the domain tier and 83.8 % overall against a 70 % target. The full table is in the root README under [Where this stands](../README.md#where-this-stands).
