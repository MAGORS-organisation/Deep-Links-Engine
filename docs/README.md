...
The documents describe the design as built and do not claim more than has been checked. As of this revision, on the build machine: the .NET solution (14 projects) builds with `-warnaserror` at 0/0; 1 419 unit, 75 contract and 850 security tests pass; both hosts start; the web SDK passes 145 tests at 8.85 kB gzip; the admin console builds. Written but **not** verified here: the 97 integration tests (need Docker), the EF Core migration (never applied to a live PostgreSQL), the Android and iOS SDKs (never compiled — CI is their first build), the k6 load profile (never run), and the 8-device manual matrix (pending). Coverage is ≥ 90 % in the domain tier and 47 % overall against a 70 % target. The full table is in the root README under [Where this stands](../README.md#where-this-stands).
