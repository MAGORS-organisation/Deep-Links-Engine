...
> The iOS SDK has not yet been compiled on a developer machine; the `sdk-ios` CI workflow is its first
> build. Treat the first release as a candidate.
...
So the engine offers three **deterministic** paths on iOS and one honest probabilistic one:
...
app and OS versions, and the claim code or login key when you have one. The result is persisted; later
calls return it without a request, and the server rate-limits the endpoint to 5 per hour per install.
...
expires after one hour; an expired code returns a typed error with `can_reissue` so you can offer a
fresh one ([api.md](api.md#problem-details)).
...
- Inside Instagram, Facebook and TikTok webviews a Universal Link fires only on a real user tap. That is
  why the engine serves an interstitial page with an actual anchor element rather than a redirect
  ([ADR-0009](../adr/0009-http-response-shape-never-301.md)).

## Consent and privacy

`DleConsent` is persisted and sent with every resolve. With `attribution == false`, or with the
