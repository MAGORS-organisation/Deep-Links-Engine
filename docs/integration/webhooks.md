...
#    "signing_key_id":"k_2026_09"}    ← the kid whose public key is in /.well-known/jwks.json
curl -s -X POST https://go.example.com/api/v1/webhooks/<ID>/test -H "Authorization: Bearer dle_…"
curl -s "https://go.example.com/api/v1/webhooks/deliveries?subscription_id=<ID>" -H "Authorization: Bearer dle_…"
```
...
DLE-Signature: t=1756900000, v1=<base64 HMAC-SHA256>, v2=<base64 Ed25519>, kid=k_2026_09
...
| `v1` | Base64 (standard alphabet, padded) of `HMAC-SHA256(secret, signing_input)` — the symmetric slot; `secret` is the subscription secret |
...
public static bool VerifyDleWebhook(
    string signatureHeader, ReadOnlySpan<byte> body, byte[] secret,
...
export function verifyDleWebhook(headerValue, rawBody, secret, jwks, now = Date.now()) {
  ...
  const v1 = crypto.createHmac("sha256", secret).update(input).digest();
...
```json
{ "keys": [ { "kty": "OKP", "crv": "Ed25519", "kid": "k_2026_09", "alg": "EdDSA", "use": "sig", "x": "<base64url 32 bytes>" } ] }
```

Keys rotate every `Dle:Crypto:KeyRotationDays` (default 90). During rotation the document lists the old and the new key, so cache it for at most an hour and re-fetch on an unknown `kid` before rejecting. (The key-rotation defect the test suite found — the bootstrap key could not be retired — is fixed; a retired `kid` disappears from JWKS and its signatures must then be rejected.)
...
After the last attempt the delivery is **dead-lettered**: it stays in `GET /api/v1/webhooks/deliveries` with `status: dead_lettered`, `attempt`, `response_code`, `last_error` and the full `payload`, and the instance-wide gauge `dle_webhook_dlq_size` goes above zero — which is an alert ([§C.6](../zadanie.md#c6-pozorovateľnosť), [operations/runbook.md](../operations/runbook.md)). Replay from the DLQ is an operator action from the console or by re-issuing from the stored payload; re-deliveries carry the original event `id`.
